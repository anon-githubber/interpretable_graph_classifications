...
if __name__ == '__main__':
	...
	for method in config["interpretability_methods"].keys():
		...
		for metric in metrics:
			score, sd = measure_fidelity(classifier_model, deepcopy(sampled_graphs), output, dataset_features)
			print("Measured %s for %s + %s: %.5f ± %.5f" % (metric, cmd_args.gm, method, score, sd))
